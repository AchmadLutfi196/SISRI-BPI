<x-app-layout>
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-6">
            ...
            <p class="text-sm text-gray-500 mt-1">Edit data ruangan {{ $ruangan->nama }}</p>
        </div>

        <!-- Form -->
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <form action="{{ route('admin.ruangan.update', $ruangan) }}" method="POST" class="p-6 space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="nama" name="nama" value="{{ old('nama', $ruangan->nama) }}" required
                ...
                <div>
                    ...
                    <input type="number" id="kapasitas" name="kapasitas" value="{{ old('kapasitas', $ruangan->kapasitas) }}" min="1"
                        class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                        placeholder="20">
                    @error('kapasitas')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
